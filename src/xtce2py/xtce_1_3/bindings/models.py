...
class ChecksumTypeName(Enum):
    ...
    :cvar VERHOEFF:
    :cvar DAMM:
    :cvar CUSTOM: Document a custom checksum algorithm in the
        InputAlgorithm element.
    """
...
class ConcernLevelsType(Enum):
    ...
    :cvar NORMAL: The case of "normal" or "no concern level" is
        generally the default.  This value can be useful when describing
        an exception or disabling when the more typical case is a non-
        normal concern level.
    :cvar WATCH: DEPRECATED: The lowest level of concern.  Systems that
        support only 3 or 4 concern levels have been observed to promote
        "watch" to "warning" during data processing, if this enumeration
        is not explicitly supported.  This value may not exist in future
        versions of this specification.
    :cvar WARNING: A level of concern to be interpreted by the user as
        less than the highest possible concern.  This is intended by the
        specification to be quite vague.  The project operational
        concept will explicitly define how these are to be used.
    :cvar DISTRESS: A level of concern to be interpreted by the user as
        greater than the least concern but not yet rising to the highest
        possible concern.  This is intended by the specification to be
        quite vague.  The project operational concept will explicitly
        define how these are to be used.
    :cvar CRITICAL: A level of concern to be interpreted by the user as
        the highest possible concern.  This is intended by the
        specification to be quite vague.  The project operational
        concept will explicitly define how these are to be used.
    :cvar SEVERE: DEPRECATED: The highest level of concern.  Systems
        that support only 3 or 4 concern levels have been observed to
        demote "severe" to "critical" during data processing, if this
        enumeration is not explicitly supported.  This value may not
        exist in future versions of this specification.
    """
...
class ConstantType:
    ...
    :ivar constant_name: Supply a name for the constant to be used to
        access this value from within the algorithm.
    :ivar value: Supply the constant value in the form of the data type
...
class SystemTypeType(Enum):
    ...
    :cvar ASSET: An form of asset monitored and/or controlled by the
        enterprise that may participate in a larger group and may be
        subdivided into internal components.
    :cvar ASSET_GROUP: A grouping of assets that make sense to aggregate
        together in the data model, such as a fleet or constellation.
    :cvar ASSET_COMPONENT: Internal systems of assets permit managing
        the structure of XTCE documents by decomposing the internal
        structures of interest to tighten the scope of an individual
        SpaceSystem element.  The XInclude facility is also available at
        the SpaceSystem element for managing the size of XTCE documents,
        in addition to the internal organization.
    :cvar UNKNOWN: The default enumeration is meant for backwards
        compatibility with earlier versions and should be avoided.
    """
...
class TimeUnitsType(Enum):
    ...
    It is best practice to avoid days, months, and years due to
    ambiguity involving leap seconds and leap days.  If these are used,
    the system should document how the leaps are handled.
    """
...
class UnitFormType(Enum):
    ...
    values.  Uncalibrated and raw here are typically synonymous, but
    there are exceptions.

    :cvar CALIBRATED: The unit of measure for this value refers to the
        engineer/calibrated value.
    :cvar UNCALIBRATED: The unit of measure for this value refers to the
        pre-calibrated data, after extraction from the data stream, when
        in the local native data type.  This is unusual, but present in
        some cases.
    :cvar RAW: The unit of measure for this value refers to the raw
        binary value from the data stream, prior to conversion to the
        local native data type and application of calibrators.
    """
...
class ArgumentRestrictionListType:
    """
    Defines a list of argument values that restrict a constraint from being
    realized in the commanding lifecycle.

    :ivar argument_restriction: Specifies an argument value that causes
        this constraint to be realized.
    """
...
class Crctype:
    ...
    :ivar polynomial: The polynomial that represents the calculation in
        hexadecimal form (described at CRCType annotation).
    :ivar init_remainder: A hexadecimal digit string specifying the
        initial value to set in the shift register before computing the
        CRC.
    :ivar final_xor: A hexadecimal digit string specifying the value to
        be added to the final shift register value before output.
    :ivar width: The width is the number of bits in the shift register,
        which is not necessarily the number of bits of the parameter
        holding the value.
    :ivar reflect_data: Endianness of the input data, true=little,
        false=big.
    :ivar reflect_remainder: Endianness of the output data, true=little,
        false=big.
    :ivar direction: The direction to perform the CRC calculation.
    :ivar bits_from_reference: An offset of non-zero may be specified to
        skip some bits against the reference position in the reference
        attribute.
    :ivar reference: The bits involved in the calculation may start at
        the beginning or the end of the container.
    :ivar parameter_ref: Reference to the parameter that contains the
        value of the CRC based on this container.  This attribute is
        optional because not all implementations verify (telemetry) or
        create (telecommand) error control fields using the XTCE
        definition.
    """
...
class MultiRangeType(FloatRangeType):
    ...
    :ivar range_form: A value of outside specifies that the most severe
        ...
        +critical +severe.  This means each min, max pair are a range:
        (-inf, min) or (-inf, min], and [max, inf) or (max, inf).
        However a value of inside "inverts" these bands: -normal -watch
        -warning -distress -critical severe +critical +distress +warning
        +watch, +normal.  This means each min, max pair form a range of
        (min, max) or [min, max) or (min, max] or [min, max].  The most
        common form used is "outside" and it is the default.  The set
        notation used defines parenthesis as exclusive and square
...
class ParityType:
    ...
    :ivar type_value: The parity form.
    :ivar bits_from_reference: An offset of non-zero may be specified to
        skip some bits against the reference position in the reference
        attribute.
    :ivar reference: The bits involved in the calculation may start at
        the beginning or the end of the container.
    :ivar parameter_ref: Reference to the parameter that contains the
        value of the parity based on this container.  This attribute is
        optional because not all implementations verify (telemetry) or
        create (telecommand) error control fields using the XTCE
        definition.
    """
...
class Xortype:
    ...
    :ivar reference: The bits involved in the calculation may start at
        the beginning or the end of the container.
    :ivar parameter_ref: Reference to the parameter that contains the
        value of this computed XOR based on this container.  This
        attribute is optional because not all implementations verify
        (telemetry) or create (telecommand) error control fields using
        the XTCE definition.
    """
...
class ArgumentMathOperationType:
    ...
    :ivar this_parameter_operand: Use the value of the target parameter
        in the calculation. It is the calibrated/engineering value only.
        If the raw value is needed, specify it explicitly using
    ...
    :ivar parameter_instance_ref_operand: This element is used to
        ...
        this math operation.
    :ivar argument_instance_ref_operand: This element is used to
        reference a value of any Argument from this command instance in
        this math operation.
    """
...
class ComparisonType(ParameterInstanceRefType):
    ...
    a type matching the Parameter being compared to.  For integer types
    it is base 10 form.  Floating point types may be specified in normal
    (100.0) or scientific (1.0e2) form.  The value is truncated  to use
    the least significant bits that match the bit size of the Parameter
    being compared to.
...
class AlarmMultiRangesType(BaseAlarmType):
    ...
    :ivar range: Describe any number of alarm ranges, each with its own
        ...
        range form (inside -- (min,max),[min,max), (min, max], [min,
        max], or outside -- (-inf, min) or (-inf,min] and [max, +inf) or
        (max,+inf).. Ranges may overlap, be disjoint and so forth.
        Ranges within the value spectrum non-specified are non-normal.
        The most severe range level of value within the ranges is the
        level of the alarm. Range values are in calibrated engineering
...
class ArgumentInputSetType:
    ...
    :ivar constant: Supply a local constant name and value to input to
        this algorithm.
    """
...
class ParameterToSetType(ParameterRefType):
    ...
    derivation: Optional[ArgumentMathOperationType] = field(
        default=None,
        metadata={
            "name": "Derivation",
            "type": "Element",
...
class ArgumentType(NameDescriptionType):
    ...
    :ivar initial_value: Specify as: integer data type using xs:integer,
        ...
        xs:dateTime, arrays using JSON syntax (e.g. '[1, 3, 4]', and
        aggregates using JSON syntax '{"member1": 1, "member2": "foo"}'
        ). Values must not exceed the characteristics for the data type
        or this is a validation error. Takes precedence over an initial
        value given in the data type. Values are calibrated unless there
...
class ArrayDataTypeType(NameDescriptionType):
    ...
    :ivar initial_value: Initial values for the individual elements of
        the array may be provided here at the type definition using JSON
        style array notation (e.g. [1, 2, 3]).  It may be multi-
        dimension, in which case the sequence matches the sequence of
        the Dimension elements in the DimensionList.  When provided
        here, the initialValue attributes in the type definition
...
class MemberType(NameDescriptionType):
    ...
    :ivar initial_value: Used to set the initial calibrated values of
        Parameters and Arguments.  Will overwrite an initial value
        defined for the ParameterType or ArgumentType definition
        elements.  For integer types it is base 10 form.  Floating point
        types may be specified in normal (100.0) or scientific (1.0e2)
        form.  Time types are specified using the ISO 8601 formats
        described for XTCE time data types.  Initial values for string
        types, may include C language style (\\n, \\t, \\", \\\\, etc.)
        escape sequences.
    """
...
class TimeAlarmRangesType(AlarmRangesType):
    """
    :ivar time_units: Time units, with the default being in seconds.
    """
...
class AggregateDataType(NameDescriptionType):
    ...
    :ivar initial_value: Initial values for the individual members of
        the aggregate/structure may be provided here at the type
        definition using JSON style notation (e.g. '{ "member1": 2,
        "member2": "foo" }').  When Member elements provide initialValue
        attributes, they take precedence over these since these are at
        the type definition level and the Member element acts like a
        Parameter element.  These may also recurse into members that are
        also aggregates.
    """
...
class ChecksumType:
    ...
    :ivar input_algorithm: Provided to account for an algorithm not
        otherwise listed by enumeration.  Assumed to return the computed
        checksum.
    :ivar bits_from_reference: An offset of non-zero may be specified to
        skip some bits against the reference position in the reference
        attribute.
    :ivar reference: The bits involved in the calculation may start at
        the beginning or the end of the container.
    :ivar name: Qualified list of named common checksum algorithms. If
        custom is chosen, InputAlgorithm must be set.
    :ivar hash_size_in_bits: The hashing algorithm may use a larger
        internal bucket size than the emitted value size in bits
        captured by the parameterRef attribute.
    :ivar parameter_ref: Reference to the parameter that contains the
        value of this computed checksum or hash based on this container.
        This attribute is optional because not all implementations
        verify (telemetry) or create (telecommand) error control fields
        using the XTCE definition.
    """
...
class CommandVerifierType(OptionalNameDescriptionType):
    ...
    :ivar argument_restriction_list: Optional list of argument values
        that manifest this post-transmission validation parameter value
        check.  When not present, this check applies to all instances of
        the command.
    """
    ...
    argument_restriction_list: Optional[ArgumentAssignmentListType] = field(
        default=None,
        metadata={
...
class ContextMatchType(MatchCriteriaType):
    ...
    It is possible that no match evaluates to true, which results in the
    default element being used.  It is also possible that a match can
    have an empty context change, in which case the default is replaced
    with nothing.
    """
...
class ErrorDetectCorrectType:
    ...
    :ivar checksum: Describe checksum or hash function applied to all or
        part of this container definition.
    :ivar crc: Describe a Cyclic Redundancy Check (CRC) algorithm
        applied to all or part of this container definition.
    :ivar xor: Describe an "exclusive or" (XOR) checksum applied to all
        or part of this container definition.
    :ivar parity: Describe a parity applied to all or part of this
...
class AlarmType(BaseAlarmType):
    ...
    :ivar disabled: The initial state of this alarm definition as
        delivered.  When true, this leaves the alarm definition empty
        for the parameter and also short circuits the remaining context
        matches and the default so no alarm is active on the parameter.
    """
...
class ArgumentDiscreteLookupListType:
    ...
    :ivar default_value: In the event that no lookup condition evaluates
        to true, then this value will be used.
    """
...
class CompleteVerifierType(CommandVerifierType):
    ...
    Consider that some may not participate due to argument value
    restriction, if that element is used.
    """
...
class DataEncodingType:
    ...
    :ivar error_detect_correct: DEPRECATED: Use the ErrorDetectCorrect
        element in the container elements instead.
    :ivar bit_order: Describes the bit ordering of the encoded value.
    :ivar byte_order: Describes the endianness of the encoded value.
    """
...
class TransmissionConstraintListType:
    ...
    :ivar transmission_constraint: A constraint that potentially blocks
...
class ContextSignificanceListType:
    ...
    :ivar context_significance: A Context contains a different
        significance definition and a context match.  The match takes
        precedence over any default significance when the first in the
        overall list evaluates to true.
    """
...
class VerifierSetType:
    ...
    associated with different states in command processing:
    ...
    :ivar transferred_to_range_verifier: Transferred to range means the
        command has been received to the network that connects the
        ground system to the spacecraft.  Typically, this verifier would
        come from something other than the spacecraft, such as a modem
        or front end processor.
    :ivar sent_from_range_verifier: Sent from range means the command
        has been transmitted to the spacecraft by the network that
        connects the ground system to the spacecraft.  Typically, this
        verifier would come from something other than the spacecraft,
        such as a modem or front end processor.
    :ivar received_verifier: A verifier that simply means the
        destination has received the command.
    :ivar accepted_verifier: A verifier that means the destination has
        accepted the command.
    :ivar queued_verifier: A verifer that means the command is scheduled
        for execution by the destination.
    :ivar execution_verifier: A verifier that indicates that the command
        is being executed.  An optional Element indicates how far along
        the command has progressed either as a fixed value or an
        (possibly scaled) ParameterInstance value.
    :ivar complete_verifier: A possible set of verifiers that all must
        be true for the command be considered completed.  Consider that
        some may not participate due to argument value restriction, if
        that element is used.
    :ivar failed_verifier: When true, indicates that the command failed.
...
class ContainerBinaryDataEncodingType:
    ...
    :ivar error_detect_correct: Describes the optional inclusion of an
        error detection and/or correction algorithm used with this
        container.
    :ivar size_in_bits: Number of bits this container occupies on the
        stream being encoded/decoded.  This is only needed to "force"
        the bit length of the container to be a fixed value.  In most
        cases, the entry list would define the size of the container.
    :ivar from_binary_transform_algorithm: Used to convert binary data
        to an application data type.
    :ivar to_binary_transform_algorithm: Used to convert binary data
        from an application data type to binary data.
...
class FloatDataEncodingType(DataEncodingType):
    ...
    :ivar context_calibrator_list: Calibrator to be applied to the raw
        uncalibrated value to arrive at the engineering/calibrated value
        when a MatchCriteria evaluates to true.  The first in the list
...
class RepeatType:
    ...
    This includes a Count of the number of appearances and an optional
    Offset in bits that may occur between appearances. A Count of 1
    indicates no repetition. The Offset default is 0 when not specified.

    :ivar count: Value (either fixed or dynamic) that contains the count
        of appearances for an Entry. The value must be positive where 1
        is the same as not specifying a RepeatEntry element at all.
    :ivar offset: Value (either fixed or dynamic) that contains an
        optional offset in bits between repeats of the Entry. The
        default is 0, which is contiguous. The value must be 0 or
        positive. Empty offset after the last repeat count is not
        implicitly reserved, so the parent EntryList should consider if
        these are occupied bits when placing the next Entry.
    """
...
class BinaryContextAlarmListType:
    ...
    :ivar context_alarm: A Context contains a new alarm definition and a
        context match.  The match takes precedence over any default
        alarm when the first in the overall list evaluates to true.  It
        is also possible the alarm definition is empty, in which case
        the context means no alarm defined when the match is true.
    """
...
class ContainerType(NameDescriptionType):
    ...
    binary_encoding: Optional[ContainerBinaryDataEncodingType] = field(
        default=None,
        metadata={
            "name": "BinaryEncoding",
            "type": "Element",
...
class TimeContextAlarmListType:
    ...
    A context is an alarm definition on a parameter which is valid only
    in the case of a test on the value of other parameters. Process the
    contexts in list order. See TimeContextAlarmType.
...
class ArgumentIntegerDataType(ArgumentBaseDataType):
    ...
    :ivar to_string: This element provides the implementation with
        assistance rendering the value as a string for users.
    :ivar initial_value: Default/Initial value is always given in
        calibrated form.  Specify the value as a base 10 integer.
    :ivar size_in_bits: Optional hint to the implementation about the
        size of the engineering/calibrated data type to use internally.
        Generally this can be determined by examination of the space
        required to capture the full range of the encoding, but it is
...
class ArgumentStringDataType(ArgumentBaseDataType):
    ...
    :ivar restriction_pattern: restriction pattern is a regular
        expression
    :ivar character_width: Optional hint to the implementation about the
        size of the engineering/calibrated data type to use internally.
        Generally this can be determined by examination of the encoding
        information for the string, but it is not always clear, so this
        attribute allows the extra hint when needed. A tolerant
        implementation will endeavor to always make sufficient size
        engineering data types to capture the entire range of possible
        characters.
    """
...
class FloatDataType(BaseDataType):
    ...
    :ivar to_string: This element provides the implementation with
        assistance rendering the value as a string for users.
    :ivar valid_range: The ValidRange defines the span of valid values
        for this data type.  This is a subset within the data type
        itself.  For example, an IEEE754 encoded float has almost a
        universal of possible values.  However, the known possible valid
        range may only span -180.0 to +180.0, due to the physics of the
        measurement.  So, the valid range provides additional
        ...
        parameter.  The valid range is not to be construed as an alarm
        definition and is not affected by any alarm definitions, instead
        violations of the valid range make a parameter value
        "unreasonable", as opposed to reasonable to be reported.  The
        exact effect of this state which should be of concern is
...
class IntegerDataType(BaseDataType):
    ...
    :ivar valid_range: The ValidRange defines the span of valid values
        for this data type.  This is a subset within the data type
        itself.  For example, a 16 bit unsigned integer has possible
        values from 0 to 65535.  However, the known possible valid range
        may only span 0 to 999, due to the physics of the measurement.
        So, the valid range provides additional boundary/constraint
        information beyond that of the data encoding in the range of
        possible values that are meaningful to this parameter.  The
        valid range is not to be construed as an alarm definition and is
        not affected by any alarm definitions, instead violations of the
        valid range make a parameter value "unreasonable", as opposed to
        reasonable to be reported.  The exact effect of this state which
        should be of concern is implementation dependent.
    :ivar initial_value: Default/Initial value is always given in
        calibrated form.  Specify the value as a base 10 integer.
    :ivar size_in_bits: Optional hint to the implementation about the
        size of the engineering/calibrated data type to use internally.
...
class BinaryParameterType(BinaryDataType):
    ...
    context_alarm_list: Optional[BinaryContextAlarmListType] = field(
        default=None,
        metadata={
...
class ArgumentTypeSetType:
    ...
    :ivar boolean_argument_type: Describe an argument type that has an
        ...
        enumeration.
    :ivar relative_time_argument_type: Describe an argument type that
...
class SpaceSystemType(NameDescriptionType):
    ...
    :ivar system_type: Type of the space system.  Represents what from a
        space enterprise this SpaceSystem element represents.  See the
        individual enumeration descriptions in SystemTypeType.
    :ivar asset_type: Broad name for the type of asset, such as
        spacecraft, aircraft, device, or any other that makes sense for
        the system.
